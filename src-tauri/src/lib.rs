...
#[tauri::command]
fn verify_stability(total_norm: i64, gov_c: i64) -> String {
...
/// Validate PIRTM source code (PIRTM Phase A)
#[tauri::command]
fn validate_pirtm(source: String, primes_json: String, stratum: String) -> Result<serde_json::Value, String> {
    match pirtm_compiler::compiler::validate_source(&source, &primes_json, &stratum) {
        Ok(envelope) => Ok(serde_json::to_value(envelope).unwrap()),
        Err(e) => Err(e),
    }
}
