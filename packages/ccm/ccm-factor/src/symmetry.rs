...
fn compute_lie_derivative<P: Float>(
    ...
    if generator.name.starts_with("Translation_") {
        ...
        for (k, grade_k) in grades.iter().enumerate() {
            ...
            let scaled = grade_k.clone() * scale_factor;
    ...
    fn compute_directional_derivative<P: Float>(
        ...
        let mut _result: CliffordElement<P> = CliffordElement::zero(section.dimension());

        // Simplified: shift components based on direction
        // In full CCM, this would use the connection on the manifold
        let result = grade1.clone(); // Placeholder
    ...
    fn compute_rotation_derivative<P: Float>(
        ...
        bivector.set_component(bivector_index, num_complex::Complex::one()).unwrap();
    ...
    let result = (ab - ba) * P::from(0.5).unwrap(); // Convention factor
    ...
    for (j, _grade_b) in grades_b.iter().enumerate() {
