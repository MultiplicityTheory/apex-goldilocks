...
impl<T: bytemuck::Pod> Tensor<T> {
    ...
    /// Get underlying buffer mutable reference
...
impl<T: bytemuck::Pod + 'static> Tensor<T> {
    /// Element-wise addition: self = a + b
    pub fn add(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>, b: &Tensor<T>) -> Result<()> {
        crate::ops::math::vector_add(exec, &a.buffer, &b.buffer, &mut self.buffer, self.numel())
    }

    /// Element-wise subtraction: self = a - b
    pub fn sub(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>, b: &Tensor<T>) -> Result<()> {
        crate::ops::math::vector_sub(exec, &a.buffer, &b.buffer, &mut self.buffer, self.numel())
    }

    /// Element-wise multiplication: self = a * b
    pub fn mul(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>, b: &Tensor<T>) -> Result<()> {
        crate::ops::math::vector_mul(exec, &a.buffer, &b.buffer, &mut self.buffer, self.numel())
    }

    /// Element-wise division: self = a / b
    pub fn div(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>, b: &Tensor<T>) -> Result<()> {
        crate::ops::math::vector_div(exec, &a.buffer, &b.buffer, &mut self.buffer, self.numel())
    }

    /// ReLU activation: self = max(0, a)
    pub fn relu(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>) -> Result<()> {
        crate::ops::activation::relu(exec, &a.buffer, &mut self.buffer, self.numel())
    }

    /// Sigmoid activation: self = sigmoid(a)
    pub fn sigmoid(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>) -> Result<()> {
        crate::ops::activation::sigmoid(exec, &a.buffer, &mut self.buffer, self.numel())
    }

    /// Tanh activation: self = tanh(a)
    pub fn tanh(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>) -> Result<()> {
        crate::ops::activation::tanh(exec, &a.buffer, &mut self.buffer, self.numel())
    }

    /// Absolute value: self = |a|
    pub fn abs(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>) -> Result<()> {
        crate::ops::math::abs(exec, &a.buffer, &mut self.buffer, self.numel())
    }

    /// Negation: self = -a
    pub fn neg(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>) -> Result<()> {
        crate::ops::math::neg(exec, &a.buffer, &mut self.buffer, self.numel())
    }

    /// Square root: self = sqrt(a)
    pub fn sqrt(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>) -> Result<()> {
        crate::ops::math::sqrt(exec, &a.buffer, &mut self.buffer, self.numel())
    }

    /// Reciprocal square root: self = 1/sqrt(a)
    pub fn rsqrt(&mut self, exec: &mut crate::executor::Executor, a: &Tensor<T>) -> Result<()> {
        crate::ops::math::rsqrt(exec, &a.buffer, &mut self.buffer, self.numel())
    }

    /// Sum reduction
    pub fn sum(&self, exec: &mut crate::executor::Executor, output: &mut Buffer<T>) -> Result<T> {
        crate::ops::reduce::sum(exec, &self.buffer, output, self.numel())
    }
}
    ...
    pub async fn matvec(&self, _exec: &mut crate::executor::Executor, _x: &Tensor<T>) -> Result<Self> {
        Err(Error::NotImplemented(
            "matvec temporarily disabled during MoonshineHRM refactor (Phase 2.1). Will be restored with CompiledOperation in Phase 2.2".to_string()
        ))
    }
