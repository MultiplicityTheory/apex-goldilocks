...
/// Automatic class deallocation when buffer is dropped
    /// Returns true if this is a linear buffer
    pub fn is_linear(&self) -> bool {
        self.pool == MemoryPool::Linear
    }

    /// Returns true if this is a boundary buffer
    pub fn is_boundary(&self) -> bool {
        self.pool == MemoryPool::Boundary
    }

    /// Fill buffer with a constant value
    pub fn fill(&mut self, exec: &mut crate::executor::Executor, value: T) -> Result<()> {
        crate::ops::memory::fill(exec, self, value, self.len)
    }

    /// Copy data from another buffer
    pub fn copy_from(&mut self, exec: &mut crate::executor::Executor, other: &Buffer<T>) -> Result<()> {
        ...
        crate::ops::memory::copy(exec, other, self, self.len)
    }
}
